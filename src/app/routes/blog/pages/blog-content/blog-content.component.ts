...
export class BlogContentComponent implements OnInit, OnDestroy {
  constructor(
    ...
    this.docUrl$.subscribe((newUrl) => {
      this.docUrl = newUrl;
    });
    this.blogInfo$.subscribe((newName: StBlogInfo) => {
      this.fileName = newName.value.title;
    });
  }

  private docUrl = 'Shai';
  private fileName = 'Shai';
  ...
  public downloadFile() {
    console.log('正在从：' + this.docUrl + ' 下载：' + this.fileName);

    this.httpClient
      .get(this.docUrl, { responseType: 'blob' })
      .subscribe((blob) => {
